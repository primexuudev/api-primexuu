/**
 * Fitur: Xuu AI (Gemini Web Engine)
 * Deskripsi: AI Assistant dengan dukungan Custom System Prompt
 */

module.exports = (app) => {
  app.get('/api/ai/xuuai', async (req, res) => {
    ...
    if (!q) {
      return res.status(400).json({
        status: false,
        message: 'Parameter text atau q wajib diisi!'
      });
    }

    const DEFAULT_PROMPT = `✨ kamu Xuu AI, sistem auto-intelligent assistant. ✨
...
Selalu jawab langsung inti.`;
    ...
    const SYSTEM_PROMPT = customPrompt && customPrompt.trim() 
      ? customPrompt.trim() 
      : DEFAULT_PROMPT;
    ...
    try {
      class GeminiClient {
        ...
        async init() {
          const r = await fetch("https://gemini.google.com/", {
            headers: {
              "user-agent": "Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36 Chrome/137 Mobile"
            }
          });
      ...
      const answer = await gemini.ask(q, SYSTEM_PROMPT);
